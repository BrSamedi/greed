...
ask_product_image = "🖼 Добавим фото продукта?\n" \
                    "Фото доступно для загрузки только в сжатом виде (Compress image).\n" \
...
ask_refund_reason = " Сообщите причину отмены заказа.\n" \
...
notification_order_refunded = "Ваш заказ отменен.\n" \
                              "\n" \
                              "{order}"
...
bot_info = 'Этот бот разработан для <...>'

# Help: guide
help_msg = "Инструкция к боту доступна по этому адресу:\n" \
           "https://yandex.ru"
...
success_order_refunded = "✴️ Зака #{order_id} был отменён."
